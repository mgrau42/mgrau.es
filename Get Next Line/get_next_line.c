...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/07/29 10:46:25 by mgrau             #+#    #+#             */
/*   Updated: 2020/10/03 12:29:57 by mgrau            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */


// La funcion get_next_line lee linea a linea el texto pasado a travez de un fd hasta el EOF(end of file).
// El tamaño del buffer es determinado a la  hora de compilarlo de  la siguiente forma :
// gcc -Wall -Wextra -Werror -D BUFFER_SIZE=xx get_next_line.c get_next_line.utils.c
...
int		val_ret(int fd, int num, char **str, char **line)
{
	if (num < 0)	/*En caso de error al leer se devuelve -1*/
		return (-1);
	else if ((num == 0) && (str[fd] == '\0'))	/*En caso de tratarse del EOF se de vuelve un 0*/
	{
